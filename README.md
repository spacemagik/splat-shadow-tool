...
- **Tint (warm ↔ cool)** — `0` keeps the shadow neutrally darker; positive values shift it cool/blue like ambient skylight, negative values shift it warm/amber to match golden-hour or firelit environments. The tint is applied at render time, so moving the slider **re-colors every shadow you've already painted** (and every shadow volume) live — no repainting needed. The tint you see is what gets baked on Export.
